 ...
 * @group Courier
 */
class CourierShowController extends Controller
{
    /**
    ...
    public function __invoke(Request $request, $id)
    {
        $order = Order::with([
            ...
            'courierAssignment.user:id,firstname,lastname',
        ])->findOrFail($id, [
            'id',
            'delivery_date',
        ...
        // Получаем fullname курьера
        $fullname = null;
        if ($order->courierAssignment && $order->courierAssignment->user) {
            $fullname = $order->courierAssignment->user->firstname . ' ' . $order->courierAssignment->user->lastname;
        }

 ...
 * @group Epay
 */
class EpaySuccessController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        if (isset($order_payment)) {
            DB::beginTransaction();
            try {
                $order_payment->status = OrderPaymentStatus::SUCCESS;
                $order_payment->save();

                $order_payment->order->order_status_id = OrderStatus::IN_PROCESS;
                $order_payment->order->save();

                DB::commit();
            } catch (\Exception $e) {
                DB::rollBack();
                Log::error('Epay set payment status error', ['error' => $e->getMessage()]);

                return $this->response([], __('response.epay.error'));
            }
        }

        return $this->response(null, __('response.epay.fail'));
    }
}
